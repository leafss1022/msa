...
Options:
  ...
  --data-dir PATH      Data directory used during install (default: /opt/msa)
  --service-name NAME  systemd service name (default: msa)
...
if [ -f /.dockerenv ] || grep -qaE '/docker/|/kubepods/|/containerd/' /proc/1/cgroup 2>/dev/null || [ "${MSA_RUNTIME:-}" = "docker" ]; then
...
if [ -f /etc/unraid-version ] || [ -x /usr/local/sbin/emhttp ] || [ -d /boot/config/plugins ] || echo "${UNRAID_VERSION:-}" | grep -qi unraid; then
  echo "on Unraid, remove msa from the WebGUI plugin page; application data is kept under /mnt/user/appdata/msa" >&2
  exit 1
fi

case "$(printf '%s %s %s %s %s' "${MSA_RUNTIME:-}" "${MSA_PACKAGE_RUNTIME:-}" "${MSA_PACKAGE_TYPE:-}" "${FNOS_RUNTIME:-}" "${FNOS_PACKAGE_TYPE:-}" | tr '[:upper:]' '[:lower:]')" in
  *fnos*|*fpk*)
    echo "fnOS FPK installs must be removed from fnOS / 飞牛应用中心 or the FPK package manager" >&2
    exit 1
    ;;
esac
if [ -f /etc/fnos-release ] || [ -f /etc/feiniu-release ] || [ -f /etc/fnOS-release ] || [ -e /usr/local/fnos ] || [ -e /var/packages/msa ]; then
  echo "fnOS FPK installs must be removed from fnOS / 飞牛应用中心 or the FPK package manager" >&2
  exit 1
fi
...
if [ "$PURGE" != "1" ] && [ "$KEEP_DATA" != "1" ]; then
  if [ -t 0 ]; then
    printf 'Remove MSA data directory %s? This deletes configs, database, logs, components, and zashboard. [y/N]: ' "$DATA_DIR"
    read ans || ans=""
    case "$ans" in
      y|Y|yes|YES) PURGE="1" ;;
      *) PURGE="0" ;;
...
stop_owned_component_processes
stop_pid_file "$DATA_DIR/msa.pid"
