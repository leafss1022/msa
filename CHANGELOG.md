...
- The shared YAML editor used by MosDNS, generic MSA config management, and Mihomo config management now uses VS Code Dark+ style highlighting for keys, strings, numbers, booleans, comments, anchors, and tags.
...
- Manual proxy share links remain editable in share-link mode when reopening the settings page, instead of being replaced by the generated `msa_manual.yaml` `proxies:` YAML.
...
- MSA does not automatically restart `firewalld`, `nftables`, or `ufw`. If a firewall service caches or replays rules, restart the active firewall service manually as documented in the Docker guide.
...
- MSA will not try to guess and repair these polluted configs automatically. Delete the polluted user config and recreate it instead of continuing to edit or use the bad content.
...
- Docker TUN 配置启用 `tun.auto-route`、`tun.auto-detect-interface` 和 `tun.dns-hijack`，并禁用 `tun.auto-redirect`，避免由 MSA 写入宿主机 nftables 或 policy routing。
- Docker TUN 下不再生成或暴露 `redir-port`、`tproxy-port`、`routing-mark`，初始化预检也不再检查 `7877` / `7896`。
- Docker 容器内禁用 `msa update` 和 WebUI 自更新安装；镜像升级需通过 Docker / Compose / 容器管理器完成。
...
- Docker TUN config enables `tun.auto-route`, `tun.auto-detect-interface`, and `tun.dns-hijack`, and disables `tun.auto-redirect` to avoid MSA writing host nftables or policy routing.
...
- `msa update` and WebUI self-update installation are disabled inside Docker containers; image upgrades must go through Docker, Compose, or the container manager.
...
- 新增登录页白底区域的动态 MSA Logo，保持原静态 Logo 的视觉尺寸，并加入 1000ms 延迟和 0.75x 播放节奏。
...
- fnOS FPK、Unraid PLG 的更新和卸载说明统一改为由平台管理器负责；`msa update` / `msa uninstall` 继续只面向 Linux tarball/systemd 安装。
...
- Added the animated MSA Logo to the white logo area on the login page, preserving the previous static logo's visual size with a 1000ms start delay and 0.75x playback pacing.
...
- fnOS FPK and Unraid PLG update/removal guidance now consistently points to the platform manager; `msa update` / `msa uninstall` remain Linux tarball/systemd-only.
...
- 这是一次热更新可靠性、更新状态可视化和 Mihomo 默认运行兼容性修复发布，重点解决 `msa update` 后用户配置丢失或需要重新设置、WebUI 下载更新等待阶段无反馈，以及 Mihomo GEO 初始化阻塞 / TProxy 健康检查在部分环境下启动或测速异常的问题。
...
- `msa update` 现在会优先使用显式 `--config`，否则自动从已安装的 systemd service、`MSA_DATA_DIR`、`ExecStart --config` / `-c` 等信息解析真实用户数据目录，避免热更新后启动到默认目录导致重新初始化。
- `msa update` 会复用已安装服务的 `--host` 和 `--port`，除非命令行显式传入，确保更新后的服务继续使用原监听地址和端口。
- `msa update` 只面向 Linux tarball/systemd 安装；Docker、Unraid 和 fnOS FPK 环境会提示通过各自平台管理器升级。
...
- 系统初始化和默认配置生成不再主动下载 Mihomo GEO 数据文件；用户后续手动将 `geo-auto-update` 改回 `true` 时，MSA 不会覆盖该选择，Mihomo 会按配置中的 `geox-url` 自行更新。

#### 修复

- 修复 v0.3.3/v0.3.4 中命令行 `msa update` 可能没有保留用户数据目录，导致更新后 WebUI 看起来被重置、需要重新设置的问题。
- 修复 `msa update` 在 systemd 服务里使用环境变量或短参数指定数据目录时无法正确继承的问题。
...
- This is a reliability and visibility release for self-update, update-progress feedback, and default Mihomo runtime compatibility. It focuses on preserving user configuration after `msa update`, keeping the WebUI informative while downloads are connecting or stuck, and avoiding Mihomo GEO initialization blocking / TProxy health-check issues in restricted environments.
...
- `msa update` now prefers explicit `--config`; otherwise it resolves the installed user data directory from the systemd service, `MSA_DATA_DIR`, and `ExecStart --config` / `-c` arguments, preventing updates from restarting into a default empty data directory.
- `msa update` now preserves the installed service `--host` and `--port` unless the command line explicitly overrides them.
- `msa update` is limited to Linux tarball/systemd installs; Docker, Unraid, and fnOS FPK environments are directed to their platform managers.
...
- Setup/default generation no longer downloads Mihomo GEO data files. If users later set `geo-auto-update` back to `true`, MSA will not override that choice and Mihomo will update through the configured `geox-url`.

#### Fixed

- Fixed `msa update` in v0.3.3/v0.3.4 potentially not preserving the user data directory, making the WebUI appear reset after update.
- Fixed `msa update` failing to inherit data directories declared through systemd environment variables or short `-c` arguments.
...
- 新增 Linux 卸载交互确认与自动化参数：`msa uninstall` 会在交互式终端询问是否删除数据目录，自动化场景可使用 `--purge --yes` 明确清理，或使用 `--keep-data` 明确保留数据。
...
- README 和 Linux tarball README 更新卸载说明，明确 `msa uninstall` 只面向 Linux tarball/systemd 安装，Docker、Unraid、fnOS FPK 需要使用对应平台管理器卸载。
...
- 修复卸载过程中可能只停止 `msa` 主进程、未可靠终止 Mihomo/MosDNS 子进程或残留组件进程的问题。
...
- Added interactive Linux uninstall confirmation and automation flags: `msa uninstall` asks whether to remove the data directory on an interactive terminal, while automation can pass `--purge --yes` to remove it or `--keep-data` to retain it.
...
- README and the Linux tarball README now clarify that `msa uninstall` is only for Linux tarball/systemd installs; Docker, Unraid, and fnOS FPK installs must be removed through their platform managers.
...
- Fixed uninstall cleanup so Mihomo/MosDNS child processes and leftover component processes are reliably terminated instead of only stopping the `msa` main process.
...
- Cloudflare Redirect 面向“不走代理的局域网客户端”访问用户指定 Cloudflare 盾站的直连解析优化场景。该功能依赖 msa 所在机器的本机网络、运营商路由、Cloudflare Anycast、IPv4/IPv6 可达性、域名名单质量和 MosDNS 当前配置，不保证所有环境都更快或更稳定；如遇访问变慢、IPv6 不通或规则未命中，请先停用插件并反馈扫描结果与日志。
- 本版本发布资产继续与 v0.3.2 保持一致：Linux amd64/arm64 tarball、Unraid `.txz`/`.plg`，以及从同步后的 `fnos-fpk` 分支构建的 fnOS x86/arm `.fpk` 包。

#### 新增

- 新增 `msa cloudflare-redirect` 命令行插件，并提供短别名 `msa cf-redirect`。
...
- 新增数据目录自动发现：命令会优先读取 `MSA_DATA_DIR` / `MSM_FREE_DATA_DIR`，并兼容 Unraid 配置、systemd 服务配置、`.msa` 兼容目录和常见安装目录。
...
- `msa init`、安装脚本和 WebUI 基础布局准备流程现在会确保默认 `cfyouxuan.yaml` 存在，但不会默认启用 Cloudflare Redirect。
...
- Cloudflare Redirect targets direct LAN clients that do not use the proxy and need selected Cloudflare-protected domains resolved to locally tested Cloudflare CDN IPs. Results depend on the msa host's own network path, ISP routing, Cloudflare Anycast behavior, IPv4/IPv6 reachability, domain-list quality, and the active MosDNS configuration. It is not guaranteed to improve every environment; if access becomes slower, IPv6 fails, or rules do not match, stop the plugin and report scan results and logs.
...
- Added the `msa cloudflare-redirect` CLI plugin, with `msa cf-redirect` as a short alias.
...
- Added data-directory auto-discovery from `MSA_DATA_DIR` / `MSM_FREE_DATA_DIR`, Unraid config, systemd service config, `.msa` compatibility paths, and common install directories.
...
- `msa init`, installer setup, and WebUI base layout preparation now ensure the default `cfyouxuan.yaml` exists, without enabling Cloudflare Redirect by default.
...
- 新增 Docker runtime 检测与保护：容器内禁用主机级 `msa update`、`msa uninstall` 和 systemd service install/uninstall。
...
- Unraid Settings 入口保留为独立轻量插件控制页，只提供启停、监听地址、端口、数据目录、状态和 `Open WebUI`；完整管理界面运行在独立 MSA WebUI。
...
- 修复 nftables / policy routing 应用逻辑：应用前清理旧 `table inet msa` 与重复 fwmark rule，路由使用 `replace`，清理时同时处理 IPv4/IPv6 policy route。
...
- Added Docker runtime detection and safeguards: host-level `msa update`, `msa uninstall`, and systemd service install/uninstall commands are disabled inside containers.
...
- Kept the Unraid Settings entry as a separate lightweight plugin control page for service enablement, listen address, port, data directory, status, and `Open WebUI`; the full management interface runs in the standalone MSA WebUI.
...
- Fixed nftables / policy routing application: old `table inet msa` and duplicate fwmark rules are cleared before apply, routes use `replace`, and cleanup now covers both IPv4 and IPv6 policy routes.
...
- 新增 `msa migrate` 一次性迁移命令，支持迁移旧数据目录、数据库文件、`update_info` 组件键、`msm_manual` Mihomo provider、旧 PID/日志文件与旧 nftables 表。
- Linux 安装脚本默认安装到 `/opt/msa`、`msa.service` 和 `/usr/local/bin/msa`，并保留 `/usr/local/bin/msm` CLI 兼容别名。
- 新增 `msa` Unraid 插件包与 CA 元数据，安装路径切换为 `/mnt/user/appdata/msa`、`rc.msa` 和 `/usr/local/emhttp/plugins/msa`。
