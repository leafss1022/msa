...
| `.plg` 模板 | `packaging/unraid/msa.plg.in` | ✅ 占位符 `__PLUGIN_VERSION__` 等由脚本 sed 替换 |
| 安装入口 `.plg` | 根目录 `msa.plg`（v0.2.2） | ✅ 指向 `leafss1022/msa` release |
| Slackware 包 | `dist/unraid/msa-0.2.2-x86_64-1.txz` + `.sha256` | ✅ 0.1.8→0.2.2 全部成品在册 |
| WebGUI 设置页 | `root/usr/local/emhttp/plugins/msa/msa.page` | ✅ Settings 菜单，含启停/端口/数据目录表单 |
| rc 服务脚本 | `root/etc/rc.d/rc.msa` | ✅ start/stop/restart/status，含阵列就绪等待 |
| 生命周期事件 | `event/started`、`event/stopping_svcs` | ✅ 随阵列启动/停止自动拉起/收尾 |
| CLI 兼容入口 | `root/usr/local/bin/msa` | ✅ 转发到真实二进制 |
| 源码运行时适配 | `cmd/msa/main.go` / `internal/server/handlers_update.go` | ✅ `isUnraidRuntime()` / `serverIsUnraidRuntime()` 早已内置 |
...
| 属性 | 官方含义 | 本仓库 `msa.plg` | 评价 |
|------|----------|----------------------|------|
| `name` | 插件标识 | `msa` | ✅ |
| `author` | 作者 | `luochuhan` | ✅ |
| `version` | 版本 | `0.2.2` | ✅（见 §4 版本号注记） |
| `pluginURL` | 自更新检查地址 | `raw.githubusercontent.com/.../main/msa.plg` | ✅ |
| `launch` | UI 入口 | `Utilities/msa` | ✅ 与 `.page` 的 `Menu="Utilities"` 对应 |
...
- 第二个 `<FILE Run="/bin/bash">` 内联脚本 → 建默认 `msa.cfg`、补可执行位、按 `ENABLED` 拉起 rc。✅
...
- 包名 `msa-0.2.2-x86_64-1.txz` = `name(msa)-version(0.2.2)-arch(x86_64)-build(1)`。✅ **完全符合四段式**。
...
- 落盘路径：`/usr/local/emhttp/plugins/msa/`（UI+二进制，RAM 盘，重启丢失靠 `.plg` 重装恢复）、`/etc/rc.d/rc.msa`（服务脚本）、`/usr/local/bin/msa`（CLI）。✅ 与官方「active 在 `/usr/local/emhttp/plugins/`、persistent 在 `/boot/config/plugins/`」模型一致——本插件持久态放 `/boot/config/plugins/msa/msa.cfg` + `/mnt/user/appdata/msa`。✅

### 2.3 `.page` WebGUI 页

官方惯例：`.page` 文件头部是 `Menu=`/`Title=`/`Icon=` 元信息，`---` 之后是 PHP 正文，emhttp 渲染进 WebGUI。

- `msa.page`：`Menu="Utilities"` + `Title="MSA Free"` + `Icon="msa.png"`，PHP 读写 `msa.cfg`、调用 rc 脚本、展示状态、给「Open WebUI」直链。✅ 按官方反馈，将 Utilities 下的图标入口作为插件控制页入口，PNG 图标随包安装到 `/usr/local/emhttp/plugins/msa/`。
...
- `cmd/msa/main.go:948 isUnraidRuntime()`：检测 `/etc/unraid-version`、`/usr/local/sbin/emhttp`、`/boot/config/plugins` 或 `UNRAID_VERSION` 环境变量。
...
- 因此**直接在 `main` 上维护 Unraid 发布即可**。发版时只需 `make unraid VERSION=x.y.z …`，把 `.txz`/`.plg` 传到对应 GitHub Release，并把生成的根 `msa.plg` 提交到 `main`（供 `pluginURL` 自更新检查）。
...
- msa 依赖的能力——`nft` 写 TProxy 表、`ip rule/route`、绑定 `:53`/低端口——在 root 下天然可用，**无需任何提权声明或权限文件**。
...
1. **公开且持续维护的仓库**（已满足：`leafss1022/msa`，需确保 public）。
...
6. **图标**：CA 列表用图（建议提供一个可公网访问的 PNG/SVG）；`.page` 入口图标若使用 PNG，需随包安装到 `/usr/local/emhttp/plugins/msa/`。
...
- [ ] `make unraid VERSION=0.2.x UNRAID_VERSION=0.2.x GITHUB_REPO=leafss1022/msa RELEASE_TAG=v0.2.x`
- [ ] `gh release` 上传 `.txz` + 根 `msa.plg`（详见 `packaging/unraid/README.md` 示例）
- [ ] 把生成的根 `msa.plg` 提交到 `main`（让 `pluginURL` 自更新生效）

**上 CA 商店（已基本备齐，2026-06-06 落地）：**
- [x] 仓库根放 OSI LICENSE → **GPL-3.0**（`LICENSE`）
- [x] 新增 `ca_profile.xml`（维护者资料，仓库根）
- [x] 新增 CA 插件列表条目 `packaging/unraid/ca/msa.xml`（`<Containers>`）
- [x] 准备 256×256 列表图标 `packaging/unraid/msa.png`
- [x] `.plg` + `.plg.in` 补 `support=`（GitHub Issues）
- [ ] **commit & push 上述文件到 `main`**，确认 raw 链接公网可达
- [ ] 确认仓库 `leafss1022/msa` 为 public
- [ ] 走 **ca.unraid.net/submit** 填仓库地址、过扫描查重、提交审核
- [ ]（可选）改用 Unraid 论坛支持帖：发帖后把 URL 填进 `ca/msa.xml` `<Support>` + `ca_profile.xml` `<Forum>` + `.plg` `support=`
