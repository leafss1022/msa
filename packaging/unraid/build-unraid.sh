...
chmod 0755 "$PKG_ROOT/usr/local/bin/msa"
...
${APP_NAME}: and msa Linux amd64 binary. Persistent data is stored under
${APP_NAME}: /mnt/user/appdata/msa by default.
${APP_NAME}:
...
sed \
  ...
  "${ROOT_DIR}/packaging/unraid/msa.plg.in" > "$PLG"
cp "$PLG" "$ROOT_PLG"
