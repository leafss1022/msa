...
Options:
  ...
  --data-dir PATH      msa data directory (default: /opt/msa)
...
if [ "$legacy_detected" = "1" ]; then
  if [ "${MSA_INSTALL_DETACHED:-0}" != "1" ] && command -v systemd-run >/dev/null 2>&1 && [ -d /run/systemd/system ]; then
    unit="msa-legacy-migrate-$(date +%Y%m%d%H%M%S)"
    ...
    if systemd-run --unit "$unit" --collect --setenv=MSA_INSTALL_DETACHED=1 /bin/sh "$SCRIPT_DIR/install.sh" "$@"; then
  ...
  if [ "${MSA_LEGACY_UPDATE_GRACE_SECONDS:-4}" != "0" ]; then
    sleep "${MSA_LEGACY_UPDATE_GRACE_SECONDS:-4}"
  fi
...
Environment=MSA_DATA_DIR=$DATA_DIR
