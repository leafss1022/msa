...
import-web:
	...
	rm -rf "$$tmp"; \
	echo "imported exported MSA web assets from $(WEB_EXPORT)"

build: package

package: frontend
	mkdir -p $(DIST)
	CGO_ENABLED=0 GOOS=$(GOOS) GOARCH=$(GOARCH) go build -trimpath -ldflags "-s -w -X main.version=$(VERSION)" -o $(BIN) ./cmd/msa
	rm -rf $(PACKAGE_DIR)
