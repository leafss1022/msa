...
原因：`fnos-fpk` = `main` + fnOS 专属代码（`cmd/msa/main.go`、`handlers_update.go` 等会被编进二进制）。在 `fnos-fpk` 上跑 `make package`/`make unraid` 会把 fnOS 代码混进 Linux/Unraid 二进制。
另外 `make unraid` 和 `make fnos` 都会**顺带重建 amd64 tarball**——所以谁最后跑，`dist/msa-linux-amd64.tar.gz` 就是谁的。**先在 main 出全部 Linux/Unraid 资产，最后才切 fnos-fpk 只出 fpk，且不要从 fnos-fpk 重传 amd64。**
...
cp -f dist/msa-linux-amd64.tar.gz dist/msm-free-linux-amd64.tar.gz
cmp dist/msa-linux-amd64.tar.gz dist/msm-free-linux-amd64.tar.gz   # 必须无输出
shasum -a 256 dist/msa-linux-amd64.tar.gz      > dist/msa-linux-amd64.tar.gz.sha256
shasum -a 256 dist/msm-free-linux-amd64.tar.gz > dist/msm-free-linux-amd64.tar.gz.sha256

# arm64
cp -f dist/msa-linux-arm64.tar.gz dist/msm-free-linux-arm64.tar.gz
cmp dist/msa-linux-arm64.tar.gz dist/msm-free-linux-arm64.tar.gz   # 必须无输出
shasum -a 256 dist/msa-linux-arm64.tar.gz      > dist/msa-linux-arm64.tar.gz.sha256
shasum -a 256 dist/msm-free-linux-arm64.tar.gz > dist/msm-free-linux-arm64.tar.gz.sha256

# Unraid
shasum -a 256 dist/unraid/msa-$VERSION-x86_64-1.txz > dist/unraid/msa-$VERSION-x86_64-1.txz.sha256
shasum -a 256 dist/unraid/msa.plg                   > dist/unraid/msa.plg.sha256
```

### 1b. 提交 root `msa.plg`（如有变化）

`make unraid` 会用本次 `.txz` 的哈希重写 root `msa.plg`（CA/raw 安装入口必须与已发布的 `.txz` 一致）：

```bash
git add msa.plg && git commit -m "Update Unraid manifest hash for v$VERSION txz" && git push origin main
```
...
shasum -a 256 dist/msa_${VERSION}_x86.fpk > dist/msa_${VERSION}_x86.fpk.sha256
git switch main         # 出完包切回 main
```

> `make fnos` 会自动下载 fnpack（飞牛 CDN）；网络不通会退化成无效 tar.gz，注意看输出 `DONE: dist/msa_..._x86.fpk` 才算成功。
> 抽查：`tar -xzf dist/msa_${VERSION}_x86.fpk -C /tmp/x && grep -E 'appname|display_name' /tmp/x/manifest` 应为 `appname = msa` / `display_name = MSA Free`。
...
gh release create v$VERSION --repo leafss1022/msa --title v$VERSION --notes-file CHANGELOG-or-notes.md \
  ...
  dist/unraid/msa-$VERSION-x86_64-1.txz dist/unraid/msa-$VERSION-x86_64-1.txz.sha256 \
  dist/unraid/msa.plg dist/unraid/msa.plg.sha256 \
...
旧客户端可达性（v0.2.2 升级链路）：旧仓库名 `leafss1022/msa` 经 GitHub 重定向到 `leafss1022/msa`，下载 `msm-free-linux-amd64.tar.gz` 会 301/302 到资产。
...
- [ ] `make unraid`（main，会重建 amd64 + 改 root msa.plg）
