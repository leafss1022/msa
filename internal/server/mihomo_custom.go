...
const (
	...
	mihomoGeneratedBackupRelPath  = "configs/mihomo/msa_generated.backup.yaml"
...
func normalizeMihomoUserConfigName(name string) (string, error) {
	...
	case "config.yaml", "phone_config.yaml", "msa_generated.backup.yaml", "config.yaml.backup":
...
func (a *App) mihomoCustomTemplateContent() string {
	...
	return strings.TrimRight(`# MSA 自定义 Mihomo config.yaml 模板
# 使用说明：
# 1. 下面这些字段和 MSA WebUI / MosDNS / 透明代理直接挂钩，除非您明确知道影响，否则不要删除或改端口：
`, "\n") + "#    " + protected + strings.TrimRight(`
# 2. 您可以自由修改 proxy-groups、proxy-providers、rule-providers、rules。
# 3. 如果设置 secret，MSA 会从配置中读取它并用于连接 Mihomo 控制器。
# 4. 保存后 MSA 会备份旧配置并重启 Mihomo。
...
func (a *App) mihomoProtectedFields() []string {
	...
	return append(fields, "secret 如用户设置，MSA 会读取并用于控制器认证")
}
