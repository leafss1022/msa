...
func (a *App) migrateLegacyRows() error {
	...
	if _, err := a.DB.Exec(`update config_histories set file_path=replace(file_path,'msa_manual.yaml','msa_manual.yaml') where file_path like '%msa_manual.yaml%'`); err != nil {
		return err
	}
	return nil
}
