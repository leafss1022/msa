...
func IsDockerRuntime() bool {
	mode := strings.ToLower(strings.TrimSpace(os.Getenv("MSA_RUNTIME")))
...
func DockerCleanupNetworkOnExit() bool {
	value := strings.ToLower(strings.TrimSpace(os.Getenv("MSA_DOCKER_CLEANUP_NETWORK_ON_EXIT")))
	return value == "" || value == "1" || value == "true" || value == "yes" || value == "on"
}

func DockerNetworkMode() string {
	value := strings.ToLower(strings.TrimSpace(os.Getenv("MSA_DOCKER_NETWORK_MODE")))
	switch value {
...
func IsFnOSFPKRuntime() bool {
	...
	for _, path := range []string{"/etc/fnos-release", "/etc/feiniu-release", "/etc/fnOS-release", "/usr/local/fnos", "/var/packages/msa"} {
		if fileExists(path) {
			return true
		}
	}
