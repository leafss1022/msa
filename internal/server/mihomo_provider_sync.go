...
func (a *App) syncMihomoProxyProvidersFromSetupConfig(cfg SetupConfig, username string) error {
	...
	if manual := renderMihomoManualProviderYAML(cfg.MihomoProxies); strings.TrimSpace(manual) != "" {
		rel := "configs/mihomo/proxy_providers/msa_manual.yaml"
...
func (a *App) mihomoManualProxiesFromProviders(providers map[string]map[string]any, fallback string) string {
	...
	provider, ok := providers["msa_manual"]
	if !ok {
		for _, candidate := range providers {
			if strings.Contains(strings.ToLower(stringMapValue(candidate, "path")), "msa_manual") {
				provider = candidate
				ok = true
				break
			}
	...
	providerPath := strings.TrimSpace(firstNonEmpty(stringMapValue(provider, "path"), "./proxy_providers/msa_manual.yaml"))
