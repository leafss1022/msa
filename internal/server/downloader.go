...
func patchZashboardIndex(uiDir string) error {
	...
	html := string(body)
	if strings.Contains(html, "msa-zashboard-disk-backend") {
		return nil
	}
...
const zashboardDiskAutoBackendScript = `<script id="msa-zashboard-disk-backend">
;(function () {
  try {
    ...
    var id = "msa-" + host.replace(/[^a-zA-Z0-9]/g, "-") + "-9090"
    var entry = {
      protocol: "http",
      secondaryPath: "",
      host: host,
    ...
    console.warn("msa zashboard disk backend preset failed", err)
  }
})()
</script>
`
