...
func (a *App) ensureSelfUpdateInfoRow() error {
	...
	_, err := a.DB.Exec(`insert into update_info(component,current_version,latest_version,has_update,status,phase,progress,message,event_log,error_message,download_url,release_notes,created_at,updated_at)
		select 'msa',?,?,?,?,?,?,?,?,?,?,?,?,?
		where not exists(select 1 from update_info where component='msa')`,
...
func (a *App) readSelfUpdateEvents() []selfUpdateEvent {
	var raw string
	if err := a.DB.QueryRow(`select coalesce(event_log,'') from update_info where component='msa' order by id desc limit 1`).Scan(&raw); err != nil {
		return nil
	}
...
func (a *App) writeSelfUpdateEvents(events []selfUpdateEvent) {
	...
	_, _ = a.DB.Exec(`update update_info set event_log=?,updated_at=? where component='msa'`, string(raw), nowString())
}
...
func (a *App) setSelfUpdateState(status, phase string, progress int, message, errorText, eventLevel, eventMessage string) {
	...
	_, _ = a.DB.Exec(`update update_info set status=?,phase=?,progress=?,message=?,event_log=?,error_message=?,updated_at=? where component='msa'`,
...
func (a *App) selfUpdateState() map[string]any {
	...
	row := a.DB.QueryRow(`select current_version,latest_version,has_update,status,coalesce(phase,''),progress,coalesce(message,''),coalesce(event_log,''),coalesce(error_message,''),coalesce(download_url,''),coalesce(release_notes,''),last_check_time from update_info where component='msa' order by id desc limit 1`)
...
func (a *App) handleUpdateCheck(w http.ResponseWriter, r *http.Request) {
	...
	_ = a.ensureSelfUpdateInfoRow()
	_, _ = a.DB.Exec(`update update_info set current_version=?,latest_version=?,has_update=?,status='checked',phase=?,progress=0,message=?,error_message='',download_url=?,release_notes=?,last_check_time=?,updated_at=? where component='msa'`,
...
func (a *App) handleUpdateDownload(w http.ResponseWriter, r *http.Request) {
	...
	_ = a.ensureSelfUpdateInfoRow()
	_, _ = a.DB.Exec(`update update_info set download_url=?,status='downloading',phase='connecting',progress=3,message='正在连接下载地址',error_message='',updated_at=? where component='msa'`, rawURL, nowString())
	...
	_, _ = a.DB.Exec(`update update_info set status='downloaded',phase='downloaded',progress=100,message='更新包已下载',error_message='',download_url=?,updated_at=? where component='msa'`, rawURL, nowString())
...
func (a *App) handleUpdateInstall(w http.ResponseWriter, r *http.Request) {
	...
	if os.Geteuid() != 0 {
		writeJSON(w, http.StatusOK, map[string]any{"success": false, "error": "需要 root 权限才能安装并重启 msa", "data": a.selfUpdateState()})
		return
	}
	if serverIsUnraidRuntime() {
		writeJSON(w, http.StatusOK, map[string]any{"success": false, "error": "Unraid 环境请通过插件管理页面更新 msa", "data": a.selfUpdateState()})
		return
	}
	state := a.selfUpdateState()
	rawURL := strings.TrimSpace(fmt.Sprint(state["download_url"]))
...
func (a *App) startSelfUpdateInstaller(archivePath string) error {
	...
	if serverSystemdAvailable() {
		unit := "msa-self-update-" + time.Now().Format("20060102150405")
	...
	go func() {
		...
		_, _ = a.DB.Exec(`update update_info set current_version=?,has_update=false,status='completed',phase='completed',progress=100,message='更新完成',error_message='',updated_at=? where component='msa'`, a.Version, nowString())
...
func (a *App) fetchGitHubJSON(rawURL string, dst any) error {
	...
	req.Header.Set("User-Agent", "msa/"+a.Version)
