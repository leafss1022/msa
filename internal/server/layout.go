...
func replaceLegacyManualReferences(path string) error {
	...
	text := string(b)
	replaced := strings.ReplaceAll(text, "msm_manual.yaml", "msa_manual.yaml")
	replaced = strings.ReplaceAll(replaced, "msm_manual:", "msa_manual:")
	replaced = strings.ReplaceAll(replaced, "msm_manual", "msa_manual")
	if replaced == text {
		return nil
	}
...
func migrateLegacyTextReferences(dataDir string) error {
	replacements := []struct {
		...
		{"msm_free", "msa"},
		{"msm_manual.yaml", "msa_manual.yaml"},
		{"msm_manual:", "msa_manual:"},
		{"msm_manual", "msa_manual"},
