...
func databasePath(dataDir string) string {
	...
	preferred := filepath.Join(dbDir, "msa.db")
	...
	// msa.db name is performed by the Phase 3 installer before the server starts.
...
func migrateLegacyManualProvider(dataDir string) error {
	oldProvider := filepath.Join(dataDir, "configs/mihomo/proxy_providers/msa_manual.yaml")
	newProvider := filepath.Join(dataDir, "configs/mihomo/proxy_providers/msa_manual.yaml")
...
func replaceLegacyManualReferences(path string) error {
	...
	text := string(b)
	replaced := strings.ReplaceAll(text, "msa_manual.yaml", "msa_manual.yaml")
	replaced = strings.ReplaceAll(replaced, "msa_manual:", "msa_manual:")
	replaced = strings.ReplaceAll(replaced, "msa_manual", "msa_manual")
	if replaced == text {
		return nil
	}
...
func migrateLegacyTextReferences(dataDir string) error {
	replacements := []struct {
		...
		{"table inet msm_free", "table inet msa"},
		{"msm_free", "msa"},
		{"msa_manual.yaml", "msa_manual.yaml"},
		{"msa_manual:", "msa_manual:"},
		{"msa_manual", "msa_manual"},
		{"msm-free-zashboard-disk-backend", "msa-zashboard-disk-backend"},
		{"msa", "msa"},
		{"msm.log", "msa.log"},
	}
...
func migrateLegacyLogFiles(dataDir string) error {
	for _, pair := range [][2]string{
		{"logs/msm.log", "logs/msa.log"},
		{"logs/msm-free.log", "logs/msa.log"},
		{"logs/msm.cli.log", "logs/msa.cli.log"},
		{"logs/msm-free.cli.log", "logs/msa.cli.log"},
		{"logs/msm.unraid.log", "logs/msa.unraid.log"},
		{"logs/msm-free.unraid.log", "logs/msa.unraid.log"},
	} {
...
func (a *App) ensureCompatibilityDatabaseLink() error {
	...
	// compatibility. After the msa rename there is a single canonical database
	// name (msa.db); a pre-rename database is opened in place by databasePath and
	// migrated to msa.db by the Phase 3 installer before the server starts.
	return nil
}
