...
func TestDockerRuntimeEnvOverride(t *testing.T) {
	...
	if !IsDockerRuntime() {
		t.Fatal("MSA_RUNTIME=docker should enable Docker runtime")
	}

	t.Setenv("MSA_RUNTIME", "native")
	if IsDockerRuntime() {
		t.Fatal("non-empty non-docker MSA_RUNTIME should disable Docker auto-detection")
	}
}

func TestDockerCleanupNetworkOnExitDefault(t *testing.T) {
	t.Setenv("MSA_DOCKER_CLEANUP_NETWORK_ON_EXIT", "")
	if !DockerCleanupNetworkOnExit() {
		t.Fatal("Docker network cleanup should default to enabled")
	}

	t.Setenv("MSA_DOCKER_CLEANUP_NETWORK_ON_EXIT", "false")
	if DockerCleanupNetworkOnExit() {
		t.Fatal("MSA_DOCKER_CLEANUP_NETWORK_ON_EXIT=false should disable cleanup")
	}
}

func TestDockerNetworkModeAliases(t *testing.T) {
	t.Setenv("MSA_DOCKER_NETWORK_MODE", "")
	if got := DockerNetworkMode(); got != "host-tun" {
		t.Fatalf("empty docker network mode = %q, want host-tun", got)
	}
	t.Setenv("MSA_DOCKER_NETWORK_MODE", "macvlan")
...
func TestDockerHostTunRouteFixAppliesFakeIPRoutes(t *testing.T) {
	t.Setenv("MSA_RUNTIME", "docker")
	t.Setenv("MSA_DOCKER_NETWORK_MODE", "host-tun")
	app := newTestApp(t)
...
func TestDockerHostTunRouteFixSkipsOtherModes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Setenv("MSA_RUNTIME", tt.runtime)
			t.Setenv("MSA_DOCKER_NETWORK_MODE", tt.network)
			app := newTestApp(t)
