...
func remediateSystemdResolvedStub(ctx context.Context) error {
	...
	content := "[Resolve]\nDNSStubListener=no\n"
	if err := os.WriteFile("/etc/systemd/resolved.conf.d/10-msa-disable-stub.conf", []byte(content), 0644); err != nil {
		return err
	}
...
func replaceResolvConfSymlink(target string) {
	...
	backup := fmt.Sprintf("/etc/resolv.conf.msa-backup-%d", time.Now().Unix())
