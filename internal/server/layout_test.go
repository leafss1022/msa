...
func TestNewMigratesLegacyLayout(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(providerDir, "msa_manual.yaml"), []byte("proxies: []\n"), 0644); err != nil {
		t.Fatal(err)
	}
...
`,
	...
	if _, err := os.Stat(filepath.Join(dbDir, "msa.db")); err != nil {
		t.Fatalf("msa.db should exist after migration: %v", err)
	}
	if _, err := os.Stat(legacyDB); !os.IsNotExist(err) {
		t.Fatalf("legacy db should be removed after migration, err=%v", err)
	}
	if _, err := os.Stat(filepath.Join(providerDir, "msa_manual.yaml")); err != nil {
		t.Fatalf("manual provider should be renamed: %v", err)
	}
	if _, err := os.Stat(filepath.Join(providerDir, "msa_manual.yaml")); !os.IsNotExist(err) {
	...
	if text := string(cfg); !strings.Contains(text, "msa_manual:") || strings.Contains(text, "msa_manual") {
	...
	if !strings.Contains(string(nft), "table inet msa") {
		t.Fatalf("network.nft should use msa table:\n%s", string(nft))
	}
	if strings.Contains(string(nft), "flush ruleset") {
		t.Fatalf("network.nft should not flush the global nftables ruleset:\n%s", string(nft))
	}
	if _, err := os.Stat(filepath.Join(dataDir, "logs/msa.log")); err != nil {
		t.Fatalf("msa log should be moved: %v", err)
	}
	...
	if component != "msa" {
		t.Fatalf("update_info component = %q, want msa", component)
	}
}
