...
func (a *App) handleDiagnosticsDownload(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Disposition", "attachment; filename=msa-diagnostics.json")
	_, _ = w.Write(b)
}
...
func fetchExitBody(client *http.Client, endpoint string) ([]byte, error) {
	...
	req.Header.Set("User-Agent", "Mozilla/5.0 (compatible; msa/exit-probe)")
...
func (a *App) applyNFT(ctx context.Context) (string, error) {
	...
	ignoreNetworkCommandError(ctx, &output, 8*time.Second, "nft", "delete", "table", "inet", "msa")
...
func (a *App) clearNFT(ctx context.Context) (string, error) {
	...
	var output bytes.Buffer
	ignoreNetworkCommandError(ctx, &output, 5*time.Second, "nft", "delete", "table", "inet", "msa")
...
func (a *App) nftStatus() map[string]any {
	...
	if out, err := combinedOutputWithTimeout(context.Background(), 3*time.Second, "nft", "list", "table", "inet", "msa"); err == nil {
...
func (a *App) handleLicenseStatus(w http.ResponseWriter, r *http.Request) {
	writeJSON(w, http.StatusOK, map[string]any{"success": true, "data": map[string]any{
		"edition": "free", "status": "unlocked", "is_pro": true, "features": "all", "message": "msa does not enforce paid licensing",
	}})
}
...
func dirWritable(path string) bool {
	...
	tmp := filepath.Join(path, ".msa-write-test")
