...
func serveFrontendIndex(w http.ResponseWriter, fsys fs.FS) {
	...
	html := string(body)
	if !strings.Contains(html, "msa-spa-recovery") {
...
func (a *App) serveZashboardIndex(w http.ResponseWriter, r *http.Request, path string) {
	...
	html := string(body)
	if !strings.Contains(html, "msa-zashboard-auto-backend") {
...
func (a *App) zashboardAutoBackendScript(r *http.Request) string {
	...
	return strings.ReplaceAll(strings.ReplaceAll(zashboardAutoBackendScriptTemplate, "__MSA_HOST__", urlQueryEscape(requestHostName(r))), "__MSA_SECRET__", secret)
}
...
const zashboardAutoBackendScriptTemplate = `<script id="msa-zashboard-auto-backend">
;(function () {
  try {
    if (!window.localStorage) return
    var listKey = "setup/api-list"
    var activeKey = "setup/active-uuid"
    var presetHost = decodeURIComponent("__MSA_HOST__")
    var presetSecret = decodeURIComponent("__MSA_SECRET__")
    ...
    var id = activeItem && activeItem.uuid ? activeItem.uuid : "msa-" + host.replace(/[^a-zA-Z0-9]/g, "-") + "-9090"
    var entry = {
      protocol: "http",
      secondaryPath: "",
      host: host,
    ...
    console.warn("msa zashboard backend preset failed", err)
  }
})()
</script>
`

const frontendSPARecoveryScript = `<script id="msa-spa-recovery">
;(function () {
  var reloadKey = "msa-spa-reload-at"
  function reloadOnce() {
    try {
