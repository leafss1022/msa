...
func statusHints(dataDir string, cfg Config, st State, running bool) []string {
	...
	if !running {
		hints = append(hints, "daemon is not running; use msa cloudflare-redirect start for periodic scan/apply, or run scan/apply manually")
	}
	if len(st.BestIPv4) == 0 && len(st.BestIPv6) == 0 {
		hints = append(hints, "no scanned Cloudflare IPs yet; run msa cloudflare-redirect scan")
	}
	if !st.Injected {
		hints = append(hints, "enabled=true but MosDNS is not injected yet; run msa cloudflare-redirect apply for immediate injection")
	}
	if st.DomainCount == 0 {
	...
	if configModifiedAfterApply(dataDir, st) && !st.LastApplyAt.IsZero() {
		hints = append(hints, "cfyouxuan.yaml changed after the last apply; run msa cloudflare-redirect apply to apply it immediately")
	}
	return hints
}
