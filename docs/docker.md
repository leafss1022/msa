...
这个实验镜像不会推送到 `latest`。拉取或部署 Docker 实验版时必须显式写出 `v0.4.1.0` tag。
...
- 镜像：`ghcr.io/leafss1022/msa:v0.4.1.0`
- 网络：`host`
...
2. 新建容器，镜像填写 `ghcr.io/leafss1022/msa:v0.4.1.0`。
...
| 变量 | 默认值 | 用途 |
|---|---|---|
| `MSA_IMAGE` | `ghcr.io/leafss1022/msa:v0.4.1.0` | 容器镜像 |
