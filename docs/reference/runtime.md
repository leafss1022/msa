...
本页面汇总 `msa` 的常见数据目录、主要文件结构和默认端口。具体安装步骤请先看对应平台文档：
...
| 部署方式 | 默认数据目录 |
|---|---|
| Linux tarball/systemd | `/opt/msa` |
| fnOS FPK | `/var/apps/msa/var` |
| Unraid PLG | `/mnt/user/appdata/msa` |
| Docker Compose / `docker-run.sh` 宿主机 | 默认当前目录的 `./msa-data` |
| Docker 容器内 | `/opt/msa` |
...
| 服务 | 端口 | 描述 |
|---|---|---|
| msa | `7777` | Web 管理界面（默认 `-p 7777`）；mosdns 亦复用它解析节点域名 |
| mosdns | `53` | DNS 服务入口 |
...
- 手动节点保存到 `mihomo_proxies`，生成 `configs/mihomo/proxy_providers/msa_manual.yaml`。
- `msa_manual` 在 Mihomo 中作为 `type: file` 的本地 proxy provider 注册。
