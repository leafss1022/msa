...
```text
https://github.com/leafss1022/msa/releases/download/v0.4.5.0/msa.plg
```
...
| 项目 | 路径 |
|---|---|
| 插件二进制 | `/usr/local/emhttp/plugins/msa/bin/msa` |
| 兼容命令 | `/usr/local/bin/msa` |
| 控制脚本 | `/etc/rc.d/rc.msa` |
| 插件配置 | `/boot/config/plugins/msa/msa.cfg` |
| 默认数据目录 | `/mnt/user/appdata/msa` |

Unraid 运行逻辑：

- 全新安装且尚未初始化时，只启动 `msa` 管理 WebUI。
- 完成初始化引导后，默认启用 Mihomo、MosDNS 和 nftables。
- Unraid 重启或插件服务重启后，`msa` 会按已保存状态恢复 Mihomo、MosDNS 和 nftables。
...
卸载请在 WebGUI 的插件管理页面删除 `msa` 插件。插件卸载会停止 WebUI 服务并移除插件文件，默认保留应用数据目录：
