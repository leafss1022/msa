...
| 架构 | FPK 资产 |
|---|---|
| x86 / amd64 | `https://github.com/leafss1022/msa/releases/download/v0.4.1.0/msa_0.4.1.0_x86.fpk` |
| ARM / arm64 | `https://github.com/leafss1022/msa/releases/download/v0.4.1.0/msa_0.4.1.0_arm.fpk` |
...
首次进入会显示初始化向导。完成初始化后，`msa` 会按保存状态恢复 Mihomo、MosDNS 和 nftables。
...
| 项目 | 路径 |
|---|---|
| 二进制 | `/var/apps/msa/target/msa` |
| 数据目录 | `/var/apps/msa/var` |
| systemd service | fnOS 注册的 `msa.service` |
| 默认 WebUI | `0.0.0.0:7777` |

FPK 运行需要 root 权限，因为 `msa` 需要绑定 MosDNS `:53`、写入 nftables、写入 `ip rule` / `ip route`，并管理透明代理相关网络状态。
...
这样做是为了避免 Linux tarball 安装流程在 `/opt/msa` 创建另一套安装，或绕过 fnOS 的包状态。

## 常用排查

打开 WebUI 后如果还没有完成初始化，只会启动 `msa` 管理界面；完成初始化后才会按配置恢复 Mihomo、MosDNS 和 nftables。
