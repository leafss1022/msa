...
```text
ghcr.io/leafss1022/msa:v0.4.4.0
```

This experimental image is not pushed as `latest`. To pull or deploy the Docker experimental version, explicitly use the `v0.4.4.0` tag.
...
services:
  msa:
    image: ghcr.io/leafss1022/msa:v0.4.4.0
    container_name: msa
    network_mode: host
    cap_add:
      - NET_ADMIN
...
```text
MSA_IMAGE=ghcr.io/leafss1022/msa:v0.4.4.0
...
2. Create a new container and set the image to `ghcr.io/leafss1022/msa:v0.4.4.0`.
...
|---|---|---|
| `MSA_IMAGE` | `ghcr.io/leafss1022/msa:v0.4.4.0` | Container image |
...
```bash
docker pull ghcr.io/leafss1022/msa:v0.4.4.0
docker stop msa
docker rm msa
./docker-run.sh
```
