...
```text
ghcr.io/scoltzero/msa:v0.3.9.2
```
...
services:
  msa:
    image: ghcr.io/scoltzero/msa:v0.3.9.2
    container_name: msa
    network_mode: host
    cap_add:
      - NET_ADMIN
...
services:
  msa:
    image: ${MSA_IMAGE:-ghcr.io/scoltzero/msa:v0.3.9.2}
...
```text
MSA_IMAGE=ghcr.io/scoltzero/msa:v0.3.9.2
...
2. Create a new container and set the image to `ghcr.io/scoltzero/msa:v0.3.9.2`.
...
|---|---|---|
| `MSA_IMAGE` | `ghcr.io/scoltzero/msa:v0.3.9.2` | Container image |
...
```bash
docker pull ghcr.io/scoltzero/msa:v0.3.9.2
docker stop msa
docker rm msa
./docker-run.sh
```
