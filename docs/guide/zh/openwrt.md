...
通过静态路由 + DHCP DNS，让 OpenWrt 主路由把流量引导到 msa 主机。下面用 UCI 命令演示（等价操作也可在 LuCI 网页里完成）。
...
> Telegram 等需要按 IPv6 段直连的服务，可继续用 `route6` 追加对应网段，下一跳同样指向 msa。
...
`dhcp_option 6` 表示向客户端下发的 DNS 服务器，这里指向 msa 主机。
