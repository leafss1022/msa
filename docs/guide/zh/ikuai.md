...
把爱快主路由的 DNS 与 FakeIP 流量引导到 msa 主机。
...
- **首选 DNS**：`192.168.1.2`（msa 主机 IP）
- **备用 DNS**：留空（不要填运营商/公共 DNS，否则部分客户端会绕过 msa）
...
> 爱快不同版本（3.X / 4.X）的菜单位置略有差异：3.X 在「网络设置 → 静态路由」，4.X 在「网络配置 → 静态路由」。条目较多时可整理成 CSV 后用「导入」功能批量添加；导入前把模板里的占位 IP 全部替换成你的 `msa主机IPv4` / `msa主机IPv6`。

## 第 3 步：可选补充路由

如需纳管硬编码公共 DNS，或让 Telegram / Netflix 这类按公网 IP 网段访问的服务继续回到 msa，可继续在静态路由里新增以下 IPv4 路由：
...
结果应落在 `28.0.0.0/8` 范围内，说明 DNS 已经过 msa。再用 `dig AAAA google.com` 验证 IPv6 结果落在 `f2b0::/18`。
