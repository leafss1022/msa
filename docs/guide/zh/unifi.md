# UniFi（Ubiquiti）接入 msa

在 UniFi 控制器里把 LAN 的 DNS 与 FakeIP 路由引导到 msa 主机。

## 示例环境

| 角色 | IPv4 | IPv6 |
|---|---|---|
| UniFi 网关 | `192.168.1.1` | `fd00::1` |
| msa 主机 | `192.168.1.2` | `fd00::2` |

## 第 1 步：DHCP 下发 DNS

1. 进入 **Settings → Networks**
2. 选择你的 LAN 网络进行编辑
3. 找到 **DHCP → DNS Server** 配置项
4. 将其切换为 **Manual**
5. **DNS Server 1** 填 msa 主机 IP：`192.168.1.2`
6. 只需填这一个 DNS，其余留空

## 第 2 步：FakeIP 静态路由（核心）

在路由/静态路由配置区新增两条：

IPv4：

- **目标网段**：`28.0.0.0/8`
- **下一跳**：`192.168.1.2`
- **类型**：Next Hop（或 Gateway）

IPv6：

- **目标网段**：`f2b0::/18`
- **下一跳**：`fd00::2`
- **类型**：Next Hop（或 Gateway）

> 不同控制器版本里「静态路由 / Static Routes」的位置略有差异，通常在 Settings 的 Routing / Static Routes 一节。

## 第 3 步：补充公共 DNS 与服务 IP 路由

在同一个 Static Routes 区域继续逐条添加下列 IPv4 路由，下一跳都填 msa 主机 IPv4：`192.168.1.2`。这些路由用于纳管硬编码公共 DNS，以及减少 Telegram / Netflix 解析或直连固定 IP 时绕过 msa 的问题。

| 目标网段 | 下一跳 | 说明 |
|---|---|---|
| `8.8.8.8/32` | `192.168.1.2` | 公共 DNS |
| `8.8.4.4/32` | `192.168.1.2` | 公共 DNS |
| `1.1.1.1/32` | `192.168.1.2` | 公共 DNS |
| `1.0.0.1/32` | `192.168.1.2` | 公共 DNS |
| `149.154.160.0/22` | `192.168.1.2` | Telegram |
| `149.154.164.0/22` | `192.168.1.2` | Telegram |
| `149.154.172.0/22` | `192.168.1.2` | Telegram |
| `91.108.4.0/22` | `192.168.1.2` | Telegram |
| `91.108.20.0/22` | `192.168.1.2` | Telegram |
| `91.108.56.0/22` | `192.168.1.2` | Telegram |
| `91.108.8.0/22` | `192.168.1.2` | Telegram |
| `95.161.64.0/22` | `192.168.1.2` | Telegram |
| `91.108.12.0/22` | `192.168.1.2` | Telegram |
| `91.108.16.0/22` | `192.168.1.2` | Telegram |
| `67.198.55.0/24` | `192.168.1.2` | Telegram |
| `109.239.140.0/24` | `192.168.1.2` | Telegram |
| `207.45.72.0/22` | `192.168.1.2` | Netflix |
| `208.75.76.0/22` | `192.168.1.2` | Netflix |
| `210.0.153.0/24` | `192.168.1.2` | Netflix |
| `185.76.151.0/24` | `192.168.1.2` | Netflix |

## 验证

客户端执行：

```bash
nslookup google.com   # 结果应落在 28.0.0.0/8
dig AAAA google.com   # 结果应落在 f2b0::/18
```

返回 [路由器接入总览](router-integration.md)。
