...
In the UniFi controller, steer the LAN DNS and FakeIP routes to the msa host.
...
5. Set **DNS Server 1** to the msa host IP: `192.168.1.2`
...
In the same Static Routes area, add the following IPv4 routes. Set every next hop to the msa host IPv4: `192.168.1.2`. These routes capture hardcoded public DNS and reduce Telegram / Netflix issues caused by bypassing msa through fixed public IPs.
