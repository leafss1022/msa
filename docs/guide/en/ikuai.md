...
Steer the iKuai main router's DNS and FakeIP traffic to the msa host.
...
| msa host IPv4 | `192.168.1.2` |
| msa host IPv6 | `fd00::2` |

## Step 1: DHCP DNS

Go to **Network Settings → LAN → DHCP Server (address pool)** and edit the pool:

- **Primary DNS**: `192.168.1.2` (msa host IP)
- **Secondary DNS**: leave empty (do not add an ISP/public DNS, or some clients bypass msa)
...
|---|---|---|
| `28.0.0.0/8` | `192.168.1.2` | msa FakeIP v4 |
| `f2b0::/18` | `fd00::2` | msa FakeIP v6 |

> Menu paths differ slightly between iKuai versions: 3.X under "Network Settings → Static Routes", 4.X under "Network Configuration → Static Routes". For many entries you can prepare a CSV and use the Import function; before importing, replace every placeholder IP in the template with your `msa host IPv4` / `msa host IPv6`.

## Step 3: Optional extra routes

To capture hardcoded public DNS and keep Telegram / Netflix fixed-IP traffic flowing back to msa, add these IPv4 static routes:
...
The result should fall within `28.0.0.0/8`, confirming DNS now goes through msa. Use `dig AAAA google.com` to confirm the IPv6 result is within `f2b0::/18`.
