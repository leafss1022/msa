...
Use static routes + DHCP DNS so the OpenWrt main router steers traffic to the msa host. Shown with UCI commands (the same can be done in LuCI).
...
> Append more `route6` entries for services (e.g. Telegram IPv6 ranges) with the same msa next hop as needed.
...
DHCP option `6` is the DNS server advertised to clients — here the msa host.
