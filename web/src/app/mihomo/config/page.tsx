...
function isReservedConfigName(name: string) {
  return ["config.yaml", "phone_config.yaml", "msa_generated.backup.yaml", "config.yaml.backup"].includes(name.trim().toLowerCase());
}
...
export default function MihomoConfigPage() {
  ...
  const restoreDefault = useCallback(async () => {
    if (!window.confirm("确定恢复 MSA 预设 Mihomo 配置？当前自定义 config.yaml 会进入历史备份。")) return;
    setActing(true);
    try {
        ...
        <div className="overflow-hidden rounded-xl border border-border/60 bg-card shadow-sm">
          <div className="flex items-center gap-2 border-b border-border/50 px-4 py-3">
            ...
            <div className="ml-auto flex items-center gap-1.5">
              ...
              <button onClick={restoreDefault} disabled={acting} className="rounded-lg border border-border/60 p-2 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground disabled:opacity-50" aria-label="恢复预设" title="恢复 MSA 预设配置">
