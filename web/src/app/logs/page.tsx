...
type Service = "msa" | "mosdns" | "singbox" | "mihomo";
...
const services: Service[] = ["msa", "mosdns", "singbox", "mihomo"];
...
export default function LogsPage({ initialService }: { initialService?: Service } = {}) {
  ...
  useEffect(() => {
    if (paused) return;
    const token = window.localStorage.getItem("msa_token") || "";
  ...
  const download = () => {
    window.location.href = `/api/v1/logs/${service}/download?token=${encodeURIComponent(window.localStorage.getItem("msa_token") || "")}`;
  };
