...
const GROUP_STATE_STORAGE_KEY = "msa-sidebar-group-open";
...
export function Sidebar({ collapsed = false }: { collapsed?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (!nav) return;
    const saved = Number(window.sessionStorage.getItem("msa-sidebar-scroll") || 0);
    ...
    >
      <div className="flex flex-col h-full">
        <nav
          ref={navRef}
          onScroll={(event) => {
            window.sessionStorage.setItem("msa-sidebar-scroll", String(event.currentTarget.scrollTop));
          }}
